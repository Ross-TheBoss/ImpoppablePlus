...
internal static class CashCalculator {
    private static readonly Dictionary<string, (int normal, int round80)> cashMap = new()
    {
        ...
        {"Bloonarius3", (100000, 100000)},
        {"BloonariusElite3", (1000000, 1000000)}
    };


    private static double IncomeMultiplier(int round)
    {
        return SimulationPatches.getMasteryMultipliers(SimulationPatches.roundsEarly).First(range => (round >= range.start) && (round <= range.end)).multiplier;
    }
    ...
    public static void OnUpdate(){
        ...
        if ((gameModel != null) && (!outputtedRoundSet)){
            ...
            if (roundSet != null){
                ModHelper.Msg<ImpoppablePlus>($"Roundset: {roundSet.name}");
                int cumulativeCash = 650;
                for (int index = 0; index < roundSet.rounds.Length; index++){
                    RoundModel round = roundSet.rounds.ElementAt(index);
                    int poppingCash = round.groups.Select(group => CashFromBloon(MasteryRounds.PromoteBloon(group.bloon), index+1) * group.count)
                                                  .Sum();
                    ...
                    ModHelper.Msg<ImpoppablePlus>($"{index+1}: ${roundCash} ${cumulativeCash}");
                }
